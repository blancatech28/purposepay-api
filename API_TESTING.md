...
* **Media Handling:** Ensure you have created the folders `profile_pics`, `vendor_ids`, `vendor_certificates`, and `vendor_locations` in your base directory to avoid file upload errors.
...
* **Body (Multipart/Form-Data):**
  ...
  * *File Fields (Select files from your computer):*

    * `verification.owner_id_document`: (Upload a dummy PDF/IMG)
    * `verification.business_registration_document`: (Upload a dummy PDF/IMG)
    * `verification.business_location_image`: (Upload a dummy JPG/PNG)

### B. Admin Approval (Simulation)

Since we are testing API flow, we must simulate the Admin approving the vendor.

**Actor:** Admin (Superuser)

1.  Create a superuser via terminal: `python manage.py createsuperuser`
2.  Login to get the Admin Token.

* **Endpoint:** `POST /vendor/admin/<vendor_id>/approve/`
* **Headers:** `Authorization: Token <admin_token>`
* **Note:** Replace `<vendor_id>` with the ID returned in step 2A.
...
* **Endpoint:** `PUT /voucher/redemptions/<redemption_id>/confirm/`
* **Headers:** `Authorization: Token <customer_token>`
* **Body:** (Empty JSON `{}` is fine, or include `"redemption_status": "REDEEMED"`).
...
5.  [ ] **Concurrency:** (Advanced) If the Vendor clicks "Redeem" twice rapidly, does it deduct twice? (Row-locking should prevent this).
...
8.  [ ] **Role Enforcement:** Customers cannot access Admin or Vendor-only endpoints, Vendors cannot access Customer-only endpoints.
...
21.  [ ] **Simultaneous Users:** Multiple users redeeming or activating vouchers simultaneously maintain correct balances.
22.  [ ] **Unexpected Inputs:** Special characters in usernames, voucher codes, and other inputs are handled gracefully.
